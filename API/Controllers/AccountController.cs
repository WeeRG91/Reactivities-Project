...
namespace API.Controllers
{
    public class AccountController(SignInManager<User> signInManager, IEmailSender<User> emailSender, IConfiguration configuration) : BaseApiController
    {
        ...
        public async Task<ActionResult> LoginWithGitHub(string code)
        {
            ...
            // step 3 - get user email if needed
        ...
        public async Task<ActionResult> ConfirmEmail([FromQuery] string userId, [FromQuery] string code)
        {
            ...
            var decondedCode = Encoding.UTF8.GetString(WebEncoders.Base64UrlDecode(code));

            var result = await signInManager.UserManager.ConfirmEmailAsync(user, decondedCode);
        ...
        public async Task<ActionResult> ChangePassword(ChangePaswordDto changePaswordDto)
        {
            var user = await signInManager.UserManager.GetUserAsync(User);
            ...
            var result = await signInManager.UserManager
                .ChangePasswordAsync(user, changePaswordDto.CurrentPassword, changePaswordDto.NewPassword);
