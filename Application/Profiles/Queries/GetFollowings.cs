...
namespace Application.Profiles.Queries
{
    public class GetFollowings
    {
        ...
        public class Handler(AppDbContext dbContext, IMapper mapper, IUserAccessor userAccessor) : IRequestHandler<Query, Result<List<UserProfileDto>>>
        {
            public async Task<Result<List<UserProfileDto>>> Handle(Query request, CancellationToken cancellationToken)
            {
                ...
                {
                    case "followers":
                        profiles = await dbContext.UsersFollowings
